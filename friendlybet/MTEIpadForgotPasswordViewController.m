//
//  MTEIpadForgotPasswordViewController.m
//  friendlybet
//
...
@interface MTEIpadForgotPasswordViewController ()
...
@implementation MTEIpadForgotPasswordViewController
...
- (void)viewDidLoad
{
    ...
    self.instructionTextField = [[UITextView alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)-200, (self.view.frame.size.height/2) - 200,400, 120)];
    ...
    self.emailLabel = [[UITextField alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)-200, (self.view.frame.size.height/2) - 40,400, 40)];
    UIImage *fieldBGImage = [UIImage imageNamed:@"Field_User.png"];
    [self.emailLabel setBackground:fieldBGImage];
    
    self.sendEmail = [[UIButton alloc] initWithFrame:CGRectMake((self.view.frame.size.width/2)-200, (self.view.frame.size.height/2)+30,400,40)];
    ...
    self.emailLabel.font = [UIFont systemFontOfSize:22];
...
- (void)sendForgotEmail
{
    ...
    if ([self NSStringIsValidEmail:self.emailLabel.text]) {
        ...
        [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://mangostatecnologia.com/secureLogin/includes/mail_forgot.php"]]];
