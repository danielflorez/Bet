//
//  MTEIpadGroupMatchesTableViewController.m
//  friendlybet
//
...
@interface MTEIpadGroupMatchesTableViewController ()
...
@implementation MTEIpadGroupMatchesTableViewController
...
{
    MTEIpadBetViewController *bvc = [[MTEIpadBetViewController alloc] init];
    bvc.match = [self.matches objectAtIndex:indexPath.row];
    bvc.event = self.event;
    bvc.gmvc = self;
    [self.ipvc.navigationController pushViewController:bvc
                                         animated:YES];
...
{
    UITableViewCell *cell = [[UITableViewCell alloc] init];
    [cell setFrame:CGRectMake(0, 0, self.tableView.frame.size.width, 149)];
    cell.backgroundColor = [UIColor clearColor];
    return cell;
}
